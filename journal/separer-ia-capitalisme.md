...
Tout d'abord commencons par une présentation sur la forme.
Ceci est un article de blog qui a surtout pour entention d'organiser ma réflexion sur divers thèmes.
Je transformerai sans doute ce texte en quelque chose de plus documenter avec un travail de références une fois que j'aurai une idée claire sur le dispositif.
...
Cela peut être interrant de partager comment les mots-clés s'articulent dans ma tête et comment cela oriente mes lectures.
...
Sans trop creuser, il y a même un lien avec l'objet du mouvement social : la retraite, càd la fin du travail.
...
Ce qu'on appelle intelligence artificielle dans ce contexte sont des *modèles de langage*.

Dans le prolongement, certains parlent de *general artificial intelligence*.
C'est un terme qui est quelque part utilisé comme un horizon pour la recherche scientifique et une diversion pour ne par parler des sujets qui fachent.

Un modèle de langage est grosso modo l'assemblage d'un dispositif d'entrainement qui produit un modèle pré-entrainé et d'un dispositif d'inférence qui utilise le produit prédécent pour calculer une prédiction.
...
Comprender et expliquer sont des taches qui sont en dehors de ce jeu.
...
C'est un peu comme un mentalisme qui va lire très rapidement des signaux corporels ou autres pour deviner une question qu'il a lui-même poser.
Cette lecture est basée sur la mémorisation et une bibliothèque de codes culturels.

Cette structuration a au moins deux niveaux : le corpus qui confirme ce qui ressemble à une explication ou non dans les conversations passée et le *préfixe* de la requête, aka *prompt*.
...
Par exemple, quand on demande une liste, une instruction sur la forme d'une liste est ajoutée sans que cela soit visible : c'est un texte avec plusieurs lignes commencant par un signe.
Quand on demande une explication, le préfixe peut être la bonne vieille structure thèse, anti-thèse, synthèse, trois paragraphes pour chaque partie, etc.


### émergence

Les modèles de langage ne sont pas aussi simple que cela.
...
Est-ce qu'il y a donc une ligne de démarcation avec d'un côté des systèmes qui donnent l'impression de discuter avec un système informatique et de l'autre un système qui donne l'illusion d'un *échange converstionnel* ?
On pourrait parler d'émergence car cette frontière n'inscrit pas une règle en tant que tel mais délimite un changement qualitatif entre les systèmes.

C'est une question que je trouve intéressante et qui m'intéresse.
Parler d'intelligence artificiel est certainement un abus de langage dans lequel on se laisse aller.
Mais cet usage de l'artifice semble tout de même dire beaucoup sur nos dimensions sociales et culturelles.
Nos modes de cognition et d'apprentissage sont en décallage avec les modes des machines et cela permet de mieux comprendre ce qui nous caractérise.


## capitalisme

Cela étant dit, le débat et la converation ne concernent pas du tout cet aspect scientifique.
...
David Harvey, dans sa lecture marxiste, caractérise le capitalisme dans une dynamique d'exploitation/exploitant/exploité. D'autres, comme Bourdieu si mes souvenirs sont bons, préfereront la polarité domination/domininant/dominé.
...
La promesse de départ était un monde meilleur et une ligne de défense contre les mésusages des intelligences artificiels.
...
Il est temps de ramener de l'argent et donc devenir acteur de l'économie capitalisme.

Il n'est pas étonnant de retrouver Hoffman Reid parmi les investisseurs et intéressés.
Le personnage a théorisé cette pratique de création de marché économique, de croissance rapide à perte pour enfin aboutir à un monopole promettant une rente exponentielle.


### compétition

Le discours dominant est que la compétion économique est une bonne chose.
...
En investissant et en faisant un partenariat avec OpenAI, Microsoft veut essayer de faire monter la valeur de son propre moteur en offrant une meilleure qualité de réponse, une meilleure compréhenesion des requêtes et de bout en bout une meilleure expérience utilisateur.
...
De l'autre Microsoft va également voir les côuts de fonctionnement augmenter mais avec la meilleure pertinence des résultats, la valeur percue d'OpenAI va également augmenter et donc l'investissement de Microsoft (1 + 11 milliards USD).
...
Se positionner comme plateforme IA et ainsi rendre l'écosystème de l'économie numérique sur-dépendant à ses produits.
...
En renforcant ces produits ou en devenant une dépendance critique pour des d'autres entreprises, on rentre également dans des problématiques de commerce international.
Cela vaut pour l'extraction de capitaux par la taxe de revenus publicitaires autant que par l'exploitation et la fuite des diplomés.
...
Dans le même sens, la bonne ou mauvaise santé de ces entreprises ne concernent qu'une fraction du monde.
À-t'on déjà vu un service baisser ses abonnements à la mesure de ses profits ?
...
Est-ce qu'on va voir la continuation d'une sorte d'applatissement du monde et des formes culturelles ?
...
Les avancées techniquees permettent un meilleur rendement qui augmente le profit.

Avec ChatGPT, OpenAI n'a pas innové, n'a rien apporté de nouveau.
Le T et le G de GPT sont des technologies issus des travaux et investissements de Google.
...
On en revient à la machinerie de Microsoft qui multipliera la valeur de ses produits par leur juxtaposition, des effets de clotûre et de l'exploitation des données d'usage.
...
Quand le but est de produire des contenus pour ramener du traffic du moteur de recherche, le travail d'information devient secondaire.
L'information est une sorte de bonus autour de la machine publicitaire.
Que se passera-t-il quand les médias ne seront plus une source profitable d'exposition publicitaires ?
...
Cette différence peut être transformer en capital supplémentaire à accumuler.

Le tautologisme "nous vivons dans la société dans laquelle nous vivons" fait qu'on ne peut pas non plus espérer grand chose d'autre de l'exploitation de cette technologie.
...
Question qui n'est pas si trivial, personnelement, et bien avant copilot, je pense que je ne saurai pas faire grand chose sans l'accès à un moteur de recherche qui me renvoie vers stackoverflow ou la documentation des différentes bibliothèques logicielles que j'utilise au quotidien.
...
Au delà d'une question scientifique et d'une lecture des enjeux économiques, j'aimerai poser la question de faire les choses différements ou différentes en dépolarisant les conversations.
...
Maintenant que tout cela est dit, je continue à être fasciner par les usages possibles.
Je bien conscient que je m'exprime d'un point de vue privilégié.
La valeur de mon travail est multiplié par ces usages plutôt que remplacé.

J'ai demandé à plusieurs services de m'aider à écrire ce texte et c'était loin d'être satisfaisant
Par contre les correcteurs orthographiques et l'usage d'outils de synthès sur ce texte en lui-même aident à son amélioration.
...
La curiosité pour ces objets techniques est-elle une bonne chose au final ?

C'est un pari que fait plus ou moins intentionnellement les personnes qui sont en mesure de comprendre et de faire ces outils.
